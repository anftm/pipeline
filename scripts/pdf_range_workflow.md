...
- 生成 PDF 通过当前产物 SHA-256 和上游 profile 识别。身份还包含 qpdf 版本、
  ...
  policy-v4 仅扩展可证明等价的被动结构及原有坏目录目标诊断；输入、上游 profile、
  qpdf 版本未变时，已完成的 v3 optimized/unchanged/no-gain 继续有效，避免全库重评。
...
policy-v4 增加 /SpiderInfo、/Threads、目录默认色彩空间以及 /EmbeddedFiles 的完整
对象图校验；附件名称、文件说明、共享引用、流字典和原始流 SHA-256 均保持一致，
不会解包、执行或删除附件。脱离正文页树的本地 /Page 打开目标保留完整可达对象图，
不改向正文页；候选丢失或改写该目标仍拒绝。
...
需要打开密码的文件优先使用已有解密资产。数字签名、XFA、自动动作、脚本和
...
仅目录 getPageIndex 的三种已知结构错误可记录后继续：非 /Page 引用、父 Kids 不含
该引用、页字典子引用类型错误；按目录路径和精确错误消息比较候选与原件，正文渲染
遇到相同错误仍失败。
